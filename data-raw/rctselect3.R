#' Simulated Oncology Trial Dataset (3 Arms)
#'
#' A simulated dataset representing the screening and enrollment pipeline for a
#' multi-site oncology trial with 2,400 screened patients and 3 treatment arms.
#' The dataset includes variables that drive eligibility decisions, treatment
#' allocation, and post-randomization attrition — designed for testing 3-arm
#' CONSORT diagram layouts with \code{\link{selecta}}.
#'
#' @format A data frame with 2,400 rows and 17 variables:
#' \describe{
...
#'     \code{"Drug B"}, \code{"Placebo"}, or \code{NA} if not randomized).
#'     Only assigned for eligible patients.}
#'
...
#'
#' @details
#' The dataset is structured to support a 3-arm CONSORT flow:
#'
...
#' }
#'
#' \strong{Randomization:} Eligible patients are allocated 1:1:1 to Drug A,
#' Drug B, or Placebo (\code{treatment}).
#'
#' \strong{Post-randomization attrition:}
#' \itemize{
#'   \item Some patients never receive treatment
#'     (\code{received_treatment == FALSE})
#'   \item Discontinuation rates vary by arm: ~14\% (Drug A), ~22\%
#'     (Drug B), ~18\% (Placebo)
#'   \item Drug B has more adverse-event-driven discontinuation
...
#' data(rctselect3)
#'
#' # 3-arm CONSORT diagram with phases and sub-reasons
#' library(selecta)
#'
...
create_rctselect3 <- function() {
    ...
    ## ==========================================================================
    ## RANDOMIZATION: 3-ARM (eligible patients only)
    ## ==========================================================================
    ...
    ## 1:1:1 allocation
    arm_assignment <- sample(c("Drug A", "Drug B", "Placebo"),
                             n_elig, replace = TRUE)
    treatment[idx_elig] <- arm_assignment
    treatment <- factor(treatment, levels = c("Drug A", "Drug B", "Placebo"))
    ...
    ## Among those who received treatment: discontinuation varies by arm
    ## Drug A ~14%, Drug B ~22%, Placebo ~18%
    idx_treated <- idx_elig[received_treatment[idx_elig] == TRUE]

    for (i in idx_treated) {
        base_rate <- switch(as.character(treatment[i]),
            "Drug A"  = 0.14,
            "Drug B"  = 0.22,
            "Placebo" = 0.18
        )
        ...
        if (runif(1) < adj_rate) {
            ...
            reason_probs <- switch(as.character(treatment[i]),
                "Drug A"  = c(0.20, 0.20, 0.25, 0.20, 0.15),
                "Drug B"  = c(0.35, 0.15, 0.20, 0.20, 0.10),
                "Placebo" = c(0.15, 0.25, 0.30, 0.15, 0.15)
            )
...
rctselect3 <- create_rctselect3()

usethis::use_data(rctselect3, overwrite = TRUE)
