...
test_that("compute data-driven counts match manual expectations for 0-arm", {
    ...
    expect_equal(first_node$n, nrow(selectaex0))
...
test_that("compute data-driven 6-arm produces 6 endpoints", {
    ...
    flow <- enroll(selectaex6, id = "patient_id") |>
        stratify("treatment") |>
        endpoint("Final")
...
### * Endpoint Breakdown (within-box)

test_that("compute stores endpoint breakdown in node", {

    flow <- enroll(n = 500) |>
        endpoint("Final diagnosis",
                 breakdown = c("Target +" = 300, "Target -" = 200))
...
test_that("compute stores per-arm endpoint breakdown", {
...
### * Cross-Arm Reason Ordering
## Per-arm exclusion sub-reasons are harmonized to a single order shared by
## every arm, sorted by the total count across arms (descending), matching
## standard clinical reporting practice rather than sorting each arm
## independently.

test_that("per-arm exclusion reasons share a global count-ordered sequence", {

    ## Global totals: Adverse = 5 + 60 = 65; Withdrew = 50 + 10 = 60;
    ## Other = 1 + 2 = 3. Global descending order: Adverse, Withdrew, Other.
    ## Arm A's own order (Withdrew > Adverse > Other) must be overridden.
    flow <- enroll(n = 1000) |>
        allocate(labels = c("A", "B"), n = c(500, 500)) |>
        exclude("Discontinued", n = c(56, 72),
                reasons = list(c("Withdrew" = 50, "Adverse" = 5, "Other" = 1),
                               c("Adverse" = 60, "Withdrew" = 10, "Other" = 2))) |>
        endpoint("Final")

    graph <- run_compute(flow)
    side  <- graph$nodes[role == "side"]
    expect_equal(nrow(side), 2)

    expected <- c("Adverse", "Withdrew", "Other")
    for (i in seq_len(nrow(side))) {
        reas <- side$reasons[[i]]
        expect_equal(names(reas), expected)
    }
})


### * Combine Count After Per-Arm Exclusion

test_that("manual combine subtracts per-arm exclusions before merging", {

    flow <- enroll(n = 1000) |>
        stratify(labels = c("A", "B"), n = c(500, 500), label = "Stratum") |>
        exclude("Dropped", n = c(40, 60)) |>
        combine("Completers") |>
        endpoint("Final")
    ...
    expect_equal(combine_node$n, (500 - 40) + (500 - 60))  # 900
})


test_that("data-driven combine agrees with cohorts() remaining count", {

    flow <- enroll(selectaex2, id = "patient_id") |>
        exclude("Failed eligibility", criterion = eligible == FALSE,
                included_label = "Eligible") |>
        stratify("treatment", label = "Treatment") |>
