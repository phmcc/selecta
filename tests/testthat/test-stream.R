...
#'
#' Tests covering sources() and combine() — the multi-source
...
test_that("combine closes a stratify split (split-and-recombine)", {

    ## combine() may follow stratify()/allocate(), not only sources()
    flow <- enroll(n = 1000) |>
    ...
    expect_s3_class(flow, "selecta")
    combine_steps <- Filter(function(s) s$type == "combine", flow$steps)
    expect_equal(length(combine_steps), 1)
    expect_equal(combine_steps[[1]]$label, "Completers")
})


### * Full PRISMA Pipeline Chains
