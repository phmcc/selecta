...
test_that("enroll creates valid selecta object in data-driven mode", {
    ...
    expect_equal(flow$n_start, nrow(selectaex2))
...
test_that("enroll data-driven mode defaults id to first column", {
    ...
    flow <- enroll(selectaex2)
    expect_equal(flow$id, names(selectaex2)[1L])
})


### * exclude() — Exclusion Logic
...
test_that("exclude errors without criteria in data-driven mode", {

    expect_error(
        enroll(selectaex2, id = "patient_id") |> exclude("Ineligible"),
        regexp = "criterion"
    )
})
...
test_that("endpoint stores single-stream breakdown", {

    flow <- enroll(n = 500) |>
        endpoint("Final diagnosis",
                 breakdown = c("Target +" = 160, "Target -" = 340))
...
test_that("endpoint stores per-arm breakdown as list", {
